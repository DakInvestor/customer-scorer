...
export default function MobileNav({ email }: Props) {
  ...
  return (
    <>
      {/* Mobile Header Bar */}
      <div className="fixed left-0 right-0 top-0 z-40 flex h-16 items-center justify-between border-b border-slate-800 bg-[#080d19] px-4 md:hidden">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">Customer Scorer</span>
        </Link>
        <button
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-72 bg-[#080d19] p-4">
            <div className="mb-6 flex justify-between">
              <span className="text-sm font-semibold text-white">Menu</span>
              <button onClick={() => setIsOpen(false)} className="rounded-md p-1 hover:bg-slate-800">
            ...
            <nav className="space-y-1">
              <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-slate-gray">Overview</p>
              ...
              <p className="mb-2 mt-4 px-3 text-xs font-semibold uppercase tracking-wider text-slate-gray">Customers</p>
              ...
              <p className="mb-2 mt-4 px-3 text-xs font-semibold uppercase tracking-wider text-slate-gray">Resources</p>
            ...
            <div className="absolute bottom-0 left-0 right-0 border-t border-slate-800 p-4">
              <p className="mb-2 truncate text-xs text-slate-gray">{email}</p>
              <button
...
function MobileNavItem({
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block rounded-lg px-3 py-2 text-sm text-slate-gray hover:bg-forsure-blue/10 hover:text-white"
    >
      {children}
    </Link>
  );
