...
export default function ReportEventClient({ businessId }: ReportEventClientProps) {
  ...
  async function loadData() {
    try {
      ...
      if (bizData) {
        setBusiness(bizData);
        setCanSubmit(bizData.network_write_access === true);
      }
  ...
  // Not authorized view - need pro subscription
  if (!canSubmit) {
    return (
      <div className="p-6 md:p-8">
        ...
        <div className="rounded-xl border border-border bg-white p-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-charcoal">Pro Subscription Required</h3>
          <p className="mt-2 text-text-secondary">
            To report events to the network, you need a Pro or Business subscription.
          </p>
          <div className="mt-6">
            <Link
              href="/app/upgrade"
              className="rounded-lg bg-copper px-6 py-3 font-medium text-white hover:bg-copper-dark"
            >
              Upgrade to Pro
            </Link>
          </div>
